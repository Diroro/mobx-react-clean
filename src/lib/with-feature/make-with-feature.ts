 ...
 * @param selector
 */
export const makeWithFeatures = <AppFeatures extends Record<string, unknown>>(
	useAllFeatures: () => AppFeatures
) => {
	const withFeature =
		<FeatureProps, OwnProps>(
			selector: Selector<FeatureProps, OwnProps, AppFeatures>
		): InferableComponentEnhancerWithProps<FeatureProps, OwnProps> =>
		(Component) => {
			...
			const connectedComponent =
				observerComponent as unknown as ConnectedComponent<
					typeof Component,
					WrappedComponentProps
				>;

			connectedComponent.WrappedComponent = Component;
			return connectedComponent as unknown as ResolvedConnectedComponent<
				FeatureProps,
				OwnProps,
				typeof Component
			>;
...
type ResolvedConnectedComponent<
	TInjectedProps,
	TNeedsProps,
